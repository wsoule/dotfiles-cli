...
var watchCmd = &cobra.Command{
	Use:     "watch",
	GroupID: "dotfiles",
	Short:   "⏱️  Watch for changes and auto-sync",
	Long: `⏱️  Watch Mode - Automatic Synchronization
...
var watchStartCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		fmt.Println("⏱️  Starting watch mode...")
		...
		if err := os.WriteFile(pidFile, []byte(fmt.Sprintf("%d", os.Getpid())), 0644); err != nil {
			fmt.Printf("⚠️  Could not create PID file: %v\n", err)
		}
...
var watchStopCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err != nil {
			fmt.Println("⚠️  Watch mode is not running")
			return
		}
...
func checkAndSync(dotfilesDir string, noPush bool) {
	...
	if err != nil {
		fmt.Printf("⚠️  [%s] Failed to check git status\n", time.Now().Format("15:04:05"))
		return
	}
	...
	if !noPush {
		...
		if err := pushCmd.Run(); err != nil {
			fmt.Printf("⚠️  [%s] Failed to push: %v\n", time.Now().Format("15:04:05"), err)
		} else {
