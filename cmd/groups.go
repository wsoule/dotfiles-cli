...
var groupsCmd = &cobra.Command{
	Use: "groups",
	GroupID: "advanced",
	Short: "Manage package groups and tags",
	Long: `  Package Groups & Tags - Organize Your Packages
  ...
  dotfiles groups create dev git,neovim,tmux,docker`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, exists := cfg.Groups[groupName]; exists {
			return fmt.Errorf("group '%s' already exists\nTo add packages to existing group: dotfiles groups add %s <package>", groupName, groupName)
		}
...
var groupsRemoveCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if !found {
			fmt.Printf("Package '%s' not in group '%s'\n", packageName, groupName)
			return nil
		}
